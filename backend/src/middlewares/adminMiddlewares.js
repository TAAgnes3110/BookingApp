 ...
 */
const adminAuditMiddleware = (action) => async (req, res, next) => {
  try {
    const auditData = {
      action,
      adminId: req.userId,
    ...
    next(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Audit logging failed'));
  }
};
 ...
 */
const adminPermissionMiddleware = (requiredPermission) => async (req, res, next) => {
  try {
    if (requiredPermission === 'DELETE_USER' && req.params.userId === req.userId) {
      throw new ApiError(httpStatus.FORBIDDEN, 'Không thể xóa chính mình');
    }

    next();
  } catch (error) {
    next(
      error instanceof ApiError ? error : new ApiError(httpStatus.FORBIDDEN, 'Permission denied')
    );
  }
};
